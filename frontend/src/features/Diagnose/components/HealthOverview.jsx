...
const HealthOverview = ({ projectData }) => {
  ...
  const getActivityIcon = (type) => {
    ...
    switch (type) {
      ...
      case "issue_opened":
        return (
          <ExclamationTriangleIcon className={`${iconProps} text-orange-600`} />
        );
      default:
  ...
  const prStats = {
    ...
    total:
      displayData.pullRequests?.total ||
      displayData.pullRequests?.merged + displayData.pullRequests?.open ||
      0,
  };
  ...
  const issueStats = {
    ...
    total:
      displayData.issues?.total ||
      displayData.issues?.closed + displayData.issues?.open ||
      0,
  };
  ...
  const prepareCommitData = () => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      if (
        displayData.commitActivities &&
        Array.isArray(displayData.commitActivities)
      ) {
        const activityForDate = displayData.commitActivities.find(
          (activity) => {
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
      <div className="p-6 space-y-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div>
              <div className="text-sm text-gray-600 mb-2">
                {prStats.open} Active Pull Request
              </div>
              {/* PR 상태 진행률 바 */}
              <div className="flex mb-4">
                <div
                  ...
                  style={{
                    width:
                      prStats.total > 0
                        ? `${(prStats.merged / prStats.total) * 100}%`
                        : "100%",
                  }}
                ></div>
                <div
                  ...
                  style={{
                    width:
                      prStats.total > 0
                        ? `${(prStats.open / prStats.total) * 100}%`
                        : "0%",
                  }}
                ></div>
              </div>
            ...
            <div>
              ...
              <div className="flex mb-4">
                <div
                  ...
                  style={{
                    width:
                      issueStats.total > 0
                        ? `${(issueStats.closed / issueStats.total) * 100}%`
                        : "100%",
                  }}
                ></div>
                <div
                  ...
                  style={{
                    width:
                      issueStats.total > 0
                        ? `${(issueStats.open / issueStats.total) * 100}%`
                        : "0%",
                  }}
                ></div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span className="font-semibold">
                    {issueStats.closed.toLocaleString()}
                  </span>
        ...
        <div>
          ...
          {(() => {
            const activities = Array.isArray(displayData.recentActivities)
              ...
              <TimelineContainer
                ...
                renderItem={(activity) => {
                  ...
                  const title =
                    ...
                    "";
                  const author =
                    activity.author || activity.user || activity.actor || "";
                  const date =
                    activity.startDate ||
                    activity.startdate ||
                    activity.date ||
